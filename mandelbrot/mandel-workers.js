...
lockJuliaColours=0;if(!lockJuliaColours){for(i=0;i<255;i++)
...
lsteps=1;else
lsteps=steps;for(j=0;j<lsteps;j++){colours[254]=Math.floor(colours[0]);if(!lockJuliaColours)
juliaColours[254]=Math.floor(juliaColours[0]);for(i=0;i<254;i++){colours[i]=Math.floor(colours[i+1]);if(!lockJuliaColours)
...
{mousePressed=1;eventTime=performance.now();var rect=mc.getBoundingClientRect();var cx=(e.clientX-rect.left);var cy=(e.clientY-rect.top);var xnorm=(canvasWidth/2-screenX)/zoom;var ynorm=(canvasHeight/2-screenY)/zoom;var zooming=0;if((cx>zoomInBtn.x)&&(cx<zoomInBtn.x+zoomInBtn.width)&&(cy>zoomInBtn.y&&cy<zoomInBtn.y+zoomInBtn.height)){zoom+=Math.ceil(zoom/5);if(zoom>maxZoom)
zoom=maxZoom;zooming=1;}
if((cx>zoomOutBtn.x)&&(cx<zoomOutBtn.x+zoomOutBtn.width)&&(cy>zoomOutBtn.y&&cy<zoomOutBtn.y+zoomOutBtn.height)){zoom-=Math.ceil(zoom/5);if(zoom<minZoom)
zoom=minZoom;zooming=1;}
if(zooming){zoomTime=performance.now();zoom=Math.floor(zoom);screenX=Math.round(-xnorm*zoom+canvasWidth/2);screenY=Math.round(-ynorm*zoom+canvasHeight/2);updateCenter();eventOccurred=1;startRender(1,1);}}
...
{if(!e.data.finished){if(blockSize[e.data.workerID]==1){percentDone[e.data.workerID]=Math.round(e.data.lineCount/(canvasHeight/workers)*100);progress=Math.floor((percentDone[0]+percentDone[1]+percentDone[2]+percentDone[3])/4);workingText.innerHTML="<i>"+progress+"%</i>";}
return 1;}
...
var lstartLine;if(e.data.blockSize==1){finished[workerID]=1;mSegment[workerID].data.set(mdSegment[workerID]);lstartLine=Math.floor(workerID*chunkHeight);offScreenCtx.putImageData(mSegment[workerID],0,lstartLine);}else{mCoarseSegment[workerID].data.set(mdCoarseSegment[workerID]);lstartLine=Math.floor(workerID*chunkHeight/scaleFactor);coarseCtx.putImageData(mCoarseSegment[workerID],0,lstartLine);mctx.drawImage(coarse,0,0);if(pointerInZoomIn)
...
zoom=Math.floor(ldestZoom);screenX=Math.round(-xnorm*zoom+canvasWidth/2);screenY=Math.round(-ynorm*zoom+canvasHeight/2);updateCenter();}
...
zoom+=Math.max(Math.ceil(Math.log(ldestZoom-zoom)*(zoom/2000)*benchmarkTime*(movingToSaved*4+1)),1);screenX=Math.round(-xnorm*zoom+canvasWidth/2);screenY=Math.round(-ynorm*zoom+canvasHeight/2);updateCenter();}
...
return 1;var zoom_old=zoom;var xnorm=(mx*1.0-screenX)/zoom;var ynorm=(my*1.0-screenY)/zoom;if(((zoom==maxZoom)&&(event.deltaY<0))||((zoom==minZoom)&&(event.deltaY>0)))
return 1;if(event.deltaY<0)
zoom+=zoom/5;else
zoom-=zoom/5;if(zoom>maxZoom)
zoom=maxZoom;if(zoom<minZoom)
zoom=minZoom;zoom=Math.floor(zoom);screenX=Math.round(-xnorm*zoom+mx);screenY=Math.round(-ynorm*zoom+my);if(zoom>=1e13||zoom_old>=1e13){Decimal.set({precision:Math.max(40,Math.ceil(Math.log10(zoom))+10)});cx_dec=cx_dec.plus(new Decimal(mx-canvasWidth/2).times(new Decimal(1).div(new Decimal(zoom_old)).minus(new Decimal(1).div(new Decimal(zoom)))));cy_dec=cy_dec.plus(new Decimal(my-canvasHeight/2).times(new Decimal(1).div(new Decimal(zoom_old)).minus(new Decimal(1).div(new Decimal(zoom)))));}updateCenter();eventOccurred=1;killWorkers();startRender(1,1);}
function mouseMove(event)
{if((posterTime!=0)||(linkDialog.style.display=="block")||(posterDialog.style.display=="block"))
return 1;var rect=mc.getBoundingClientRect();var mx=(event.clientX-rect.left)/(rect.right-rect.left)*rect.width*scaleFactor-6;var my=(event.clientY-rect.top)/(rect.bottom-rect.top)*rect.height*scaleFactor-6;var cx=(event.clientX-rect.left);var cy=(event.clientY-rect.top);if((cx>zoomInBtn.x)&&(cx<zoomInBtn.x+zoomInBtn.width)&&(cy>zoomInBtn.y&&cy<zoomInBtn.y+zoomInBtn.height)){if((!pointerInZoomIn)&&(!needRedraw))
drawZoomButtons(1,"in");pointerInZoomIn=1;pointerInZoomOut=0;}
else if((cx>zoomOutBtn.x)&&(cx<zoomOutBtn.x+zoomOutBtn.width)&&(cy>zoomOutBtn.y&&cy<zoomOutBtn.y+zoomOutBtn.height)){if((!pointerInZoomOut)&&(!needRedraw))
drawZoomButtons(1,"out");pointerInZoomOut=1;pointerInZoomIn=0;}
else if((pointerInZoomIn)||(pointerInZoomOut)){if(!needRedraw)
drawZoomButtons(1);pointerInZoomIn=0;pointerInZoomOut=0;}
if((mx<=0)||(mx>canvasWidth)||(my<=0)||(my>canvasHeight)){centerX=canvasWidth/2;centerY=canvasHeight/2;if((showJulia)&&(!drawingJulia)){var xnorm=Math.floor((centerX-screenX)/zoom*1000000000)/1000000000;var ynorm=Math.floor((centerY-screenY)/zoom*1000000000)/1000000000;startJulia(1,xnorm,ynorm);}
...
if(zoom>=1e13&&xOffset!==undefined){Decimal.set({precision:Math.max(40,Math.ceil(Math.log10(zoom))+10)});cx_dec=cx_dec.minus(new Decimal(xOffset).div(new Decimal(zoom)));cy_dec=cy_dec.minus(new Decimal(yOffset).div(new Decimal(zoom)));}updateCenter();oldMouseX=event.clientX;oldMouseY=event.clientY;eventOccurred=1;killWorkers();startRender(1,1);}
...
return 1;var canvasCenterX=canvasWidth/2;var canvasCenterY=canvasHeight/2;centerX=Math.min(m1x,m2x)+Math.abs(m1x-m2x)/2;centerY=Math.min(m1y,m2y)+Math.abs(m1y-m2y)/2;distanceBetween=Math.sqrt(Math.pow(m1x-m2x,2)+Math.pow(m1y-m2y,2));if(firstPinchDistance!=0){pinchRatio=distanceBetween/firstPinchDistance;var zoom_old=zoom;var xnorm=(centerX*1.0-screenX)/zoom;var ynorm=(centerY*1.0-screenY)/zoom;if(((zoom==maxZoom)&&(pinchRatio>1))||((zoom==minZoom)&&(pinchRatio<1)))
return 1;if(pinchRatio>1.0)
zoom+=Math.floor((pinchRatio*zoom)/50);else
zoom-=Math.floor((pinchRatio*zoom)/50);if(zoom>maxZoom)
zoom=maxZoom;if(zoom<minZoom)
zoom=minZoom;screenX=Math.floor(-xnorm*zoom+centerX);screenY=Math.floor(-ynorm*zoom+centerY);if(zoom>=1e13||zoom_old>=1e13){Decimal.set({precision:Math.max(40,Math.ceil(Math.log10(zoom))+10)});cx_dec=cx_dec.plus(new Decimal(centerX-canvasWidth/2).times(new Decimal(1).div(new Decimal(zoom_old)).minus(new Decimal(1).div(new Decimal(zoom)))));cy_dec=cy_dec.plus(new Decimal(centerY-canvasHeight/2).times(new Decimal(1).div(new Decimal(zoom_old)).minus(new Decimal(1).div(new Decimal(zoom)))));}updateCenter();eventOccurred=1;startRender(1,1);}else
...
if(zoom>=1e13&&xOffset!==undefined){Decimal.set({precision:Math.max(40,Math.ceil(Math.log10(zoom))+10)});cx_dec=cx_dec.minus(new Decimal(xOffset).div(new Decimal(zoom)));cy_dec=cy_dec.minus(new Decimal(yOffset).div(new Decimal(zoom)));}else{updateCenter();}oldMouseX=event.targetTouches[0].clientX;oldMouseY=event.targetTouches[0].clientY;eventOccurred=1;killWorkers();startRender(1,1);}}
function touchEnd(event)
{firstPinchDistance=0;mousePressed=0;oldMouseX=-1;oldMouseY=-1;}
function drawZoomButtons(redraw,highlight)
{if(redraw){mctx.drawImage(offScreen,0,0,canvasWidth/scaleFactor,canvasHeight/scaleFactor);if(showAxes)
drawAxes();}
...
const showJulia = document.getElementById("showJulia")?.checked;
const altMandel = document.getElementById("altMandelbrotMode")?.checked;

juliaWorker.postMessage({
  juliaBuffer: julia.buffer,
  Cr: xnorm,
  Ci: ynorm,
  width: juliaCanvasWidth,
  height: juliaCanvasHeight,
  iterations: juliaIterations,
  mode: altMandel ? "mandelbrot" : (showJulia ? "julia" : "none")
}, [julia.buffer]);}
...
eventTime=performance.now();needRedraw=1;toggleJulia();toggleSmooth();updateCenter();startRender(1,1);}
...
logText.innerHTML="<i>Working...</i>";itersInput.value=iterations;juliaItersText.textContent=juliaIterations;zoomText.textContent=Math.floor(zoom);mc.style.borderColor="black";mc.style.outline="5px solid gray";if(lneedRecompute){rotating=0;cycleText.innerHTML="Animate";}
...
if(needRecompute){var usePerturb=(zoom>=1e13);if(!computeWorker[i]){computeWorker[i]=new Worker(usePerturb?"mandel-perturb-compute.js":"mandel-compute.js");computeWorker[i].onmessage=onComputeEnded;computeWorker[i]._isPerturb=usePerturb;}else if(computeWorkerRunning[i]==0&&computeWorker[i]._isPerturb!==usePerturb){computeWorker[i].terminate();computeWorker[i]=new Worker(usePerturb?"mandel-perturb-compute.js":"mandel-compute.js");computeWorker[i].onmessage=onComputeEnded;computeWorker[i]._isPerturb=usePerturb;}
workersRunning++;computeWorkerRunning[i]=1;if(blockSize[i]==1)
computeWorker[i].postMessage({mandelBuffer:mandel[i].buffer,workerID:i,startLine:startLine,blockSize:blockSize[i],canvasWidth:canvasWidth,segmentHeight:chunkHeight,screenX:screenX,screenY:screenY,zoom:zoom,cx_center:cx_center,cy_center:cy_center,cxhi:cx_hi,cxlo:cx_lo,cyhi:cy_hi,cylo:cy_lo,cx_dec_str:cx_dec.toString(),cy_dec_str:cy_dec.toString(),iterations:iterations,oneShot:0,smooth:smooth,smoothMandel:smoothMandel[i].buffer},[mandel[i].buffer],[smoothMandel[i].buffer]);else
computeWorker[i].postMessage({mandelBuffer:mandel[i].buffer,workerID:i,startLine:startLine/scaleFactor,blockSize:blockSize[i],canvasWidth:coarseWidth,segmentHeight:chunkHeight/2,screenX:screenX/scaleFactor,screenY:screenY/scaleFactor,zoom:zoom/scaleFactor,cx_center:cx_center,cy_center:cy_center,cxhi:cx_hi,cxlo:cx_lo,cyhi:cy_hi,cylo:cy_lo,cx_dec_str:cx_dec.toString(),cy_dec_str:cy_dec.toString(),iterations:iterations,oneShot:0,smooth:smooth,smoothMandel:smoothMandel[i].buffer},[mandel[i].buffer],[smoothMandel[i].buffer]);}
